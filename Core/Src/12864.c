...
uint8_t currentStep[] = {0xb5, 0xb1, 0xc7, 0xb0, 0xb2, 0xbd, 0xca, 0xfd, 0xa3, 0xba};
uint8_t cycleNumber[] = {0xd1, 0xad, 0xbb, 0xb7, 0xb4, 0xce, 0xca, 0xfd, 0xa3, 0xba};
...
void LCDInit(void)
{
    ...
    DisplayChineseCharacter(0x90,TargetSteps,size);
    size = sizeof(currentStep) / sizeof(uint8_t);
    DisplayChineseCharacter(0x88,currentStep,size);
    size = sizeof(cycleNumber) / sizeof(uint8_t);
    DisplayChineseCharacter(0x98,cycleNumber,size);
}